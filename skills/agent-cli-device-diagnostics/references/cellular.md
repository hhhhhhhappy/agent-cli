...
For authoritative per-service log pattern interpretation (e.g., `redial2`, `lqm`, `NetworkManager` diagnostics), consult [model-reference/index.md](model-reference/index.md) when a model-specific diagnostic document exists for the current device model. The model-reference covers root-cause diagnosis for individual services; this document covers the end-to-end cellular troubleshooting flow.
...
- Cellular session is healthy but the remaining question is generic uplink, route, DNS, or log review: suggest switching to `agent-cli-inspect` for targeted `status` or `log` collection.
- Repeated APN, SIM policy, dual-SIM priority, or other config mismatch is the clearest issue: suggest switching to `agent-cli-config` for schema-backed config review or change planning.
- If the user moves from diagnosis to an approved upgrade or reboot as the next recovery step, suggest switching to `agent-cli-operate`.
