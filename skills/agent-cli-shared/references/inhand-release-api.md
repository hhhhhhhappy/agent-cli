...
> **API Key note:** The `x-api-key` below is the public frontend key obtained from the
> official InHand website (`www.inhand.com`). It is a non-secret key used by the
> website's own frontend JavaScript to call the public product API. It is not a
> user-specific credential or secret token.
