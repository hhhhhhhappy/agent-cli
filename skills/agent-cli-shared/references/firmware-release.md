...
Use when the user asks about official firmware changelog, version availability, package download links, or upgrade guidance before execution, or when the current device problem may be explained by a known firmware fix published by the vendor.

Read [inhand-release-api.md](inhand-release-api.md) before making website requests, and replay the vendor API with shell `curl` instead of generic `Fetch` helpers. Use the active CLI or MCP transport (as configured via `agent-cli-shared`) for device communication.
...
- The user wants to prepare for an upgrade and needs help selecting the target official version before execution.
...
2. Read basic device info before any vendor website request:
   - Always read local `status basic` first through the active transport.
   - If the MCP environment exposes fixed resources and the exact `device_id` is known, you may prefer `device://<device_id>/status/basic`.
   - Otherwise use the normal `status basic` command path through CLI or MCP.
...
17. Execute upgrade only after explicit approval:
   ...
   - Then switch to `agent-cli-operate` for the actual upgrade execution
   - Keep the selected version, final URL, and risk notes as evidence for that handoff

## Interpretation

- `status basic` is the source of truth for the current model and installed firmware. The vendor website lookup starts only after that local read succeeds, regardless of whether the read came from CLI, MCP, or an MCP fixed resource.
